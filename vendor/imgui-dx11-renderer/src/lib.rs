...
impl Renderer {
    ...
    unsafe fn setup_render_state(&self, draw_data: &DrawData) {
        ...
        ctx.OMSetBlendState(&self.blend_state, blend_factor.as_ptr(), 0xFFFFFFFF);
...
impl StateBackup {
    unsafe fn backup(context: Option<ID3D11DeviceContext>) -> Self {
        ...
        ctx.RSGetScissorRects(&mut scissor_count, &mut result.scissor_rects);
        ctx.RSGetViewports(&mut viewport_count, &mut result.viewports);
        ctx.RSGetState(&mut result.rasterizer_state);
        ctx.OMGetBlendState(
            &mut result.blend_state,
            result.blend_factor.as_mut_ptr(),
        ...
        let mut ps_instance_count = 0;
        let mut vs_instance_count = 0;
        let mut gs_instance_count = 0;
        ctx.PSGetShader(&mut result.ps_shader, null_mut(), &mut ps_instance_count);
        ctx.VSGetShader(&mut result.vs_shader, null_mut(), &mut vs_instance_count);
        ctx.VSGetConstantBuffers(0, &mut result.constant_buffer);
        ctx.GSGetShader(&mut result.gs_shader, null_mut(), &mut gs_instance_count);
    ...
    pub fn restore(mut self) {
        unsafe {
            ...
            ctx.RSSetState(&self.rasterizer_state);
            ctx.OMSetBlendState(&self.blend_state, self.blend_factor.as_ptr(), 0xFFFFFFFF);
